...
public static class ReportService
{
    ...
    public static string? ExportMonthlyStatementPDF(BankService bank, string accountNumber, int month, int year)
    {
        ...
        lines.Add("Fecha        Tipo        Monto    Descripcion       Contraparte");
        foreach(var t in acc.Transactions.Where(t=>t.Date.Month==month && t.Date.Year==year))
            lines.Add($"{t.Date:yyyy-MM-dd HH:mm}  {t.Type,-12} {t.Amount,10:F2}  {t.Description,-15}  {t.Counterparty}");
        var pdfPath=$"Extracto_{accountNumber}_{year}-{month:00}.pdf";
        MinimalPdfWriter.WriteSinglePageText(pdfPath, lines);
...
{
    public static void WriteSinglePageText(string path, List<string> lines)
    {
        var sb = new StringBuilder();
        sb.Append("BT /F1 10 Tf 50 750 Td 12 TL ");
        ...
        var pdf = BuildPdf(contentStream);
    ...
    static byte[] BuildPdf(string content)
    {
        ...
        objects.Add(Obj(5, $"<< /Length {contentBytes.Length} >>\nstream\n{content}\nendstream"));
        ...
        for (int i = 1; i <= 5; i++)
            w.Write($"{offsets[i]:0000000000} 00000 n \n");
        w.Write("trailer\n<< /Size 6 /Root 1 0 R >>\nstartxref\n");
        w.Write(xrefPos.ToString());
        w.Write("\n%%EOF");
        w.Flush();
    ...
    static byte[] Obj(int id, string body)
    {
        var s = $"{id} 0 obj\n{body}\nendobj\n";
